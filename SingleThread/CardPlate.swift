...
///
/// All three plates share the same 10pt corner radius and the same adaptive
/// fill (off-white light / black dark). The swipe-prompt box renders as a
/// separate plate below the card rather than nested inside it, so it reuses
/// `plateFill(for:)` instead of a distinct fill.
enum CardPlate {
    ...
    /// slightly darkened orange so the caption stays legible on the off-white
    /// plate; dark mode keeps the semantic `.orange` that matches the
