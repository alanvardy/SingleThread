...
- [ ] `SIM='platform=iOS Simulator,name=iPhone 17,OS=26.2' ./scripts/test.sh` passes (format, lint, build, Periphery, unit + UI tests, watch UI tests, macOS build + unit tests)
  - ⚠️ Blocked only by a **pre-existing** failure: `testPinWallpaperTogglePersistsAcrossRelaunch` (pin-wallpaper feature, untouched by this branch) fails on this machine **and on `origin/main`** (reproduced in a `origin/main` worktree; also fails on main's CI run 33439793839). All other stages pass: format ✓, SwiftFormat check ✓, SwiftLint `--strict` ✓, iOS build ✓, watch build ✓, Periphery ✓, iOS unit tests ✓ (incl. all 16 MicrophoneToggleTests), iOS UI tests ✓ (all except the pin test), watch UI tests ✓ locally (exit 0; the `testCompleteHoldsCardDuringGlow` CI flake passes locally), macOS build + unit tests ✓ (after the Phase-3 Settings-button test was gated to iOS).
- [x] Confirm the branch has both unit coverage (Phase 1–3 tests) and that no UI-test seam is required — speech authorization has no production launch-arg seam, so dictation is unit-tested only, as designed (explicitly noted in the PR): 13 new unit tests in `MicrophoneToggleTests` cover the default registration, authorization passthrough, scene-phase refresh, and explanatory-label rendering; dictation/speech cannot be driven deterministically in UI tests (no TCC seam exists), so unit coverage is the correct and complete guard.
