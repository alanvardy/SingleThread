...
#### Automated
- [x] `./scripts/test.sh` passes once, end-to-end — EXCEPT 2 pre-existing macOS StoreKit-sandbox entitlement tests (`EntitlementStoreTests.isEntitledSurvivesStoreRecreation` / `initialRefreshSettlesResolvedFlag`) that fail only on this Mac: the local sandbox account resolves `isEntitled=true`. PASS on iOS sim in the same gate run; mac-tests green in CI; test file byte-identical to origin/main; branch diff touches zero entitlement code.
