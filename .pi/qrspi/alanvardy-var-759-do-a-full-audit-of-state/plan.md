...
- [x] `bash audit/verify-citations.sh` exits 0 (all citations pinned to source, no drift)
- [x] `bash audit/verify-citations-self-test.sh` exits 0 (verifier catches corruption)
- [x] Inventory cite-check passes: all `inventory.md` file:line refs ⊆ `factbase.tsv`
- [x] Cluster matrices fully annotated: zero bare `undefined` cells (only listed open areas)
- [x] Three enum sketches only; `replaces` fields resolve
- [x] Findings tier ordering monotonic: T1 before T2 before T3 before T4
- [x] `index.md` links all five artifacts
- [x] `./scripts/test.sh` remains green (app is untouched by design)
