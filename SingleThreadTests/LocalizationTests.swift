        ...
        for (name, url) in Self.catalogs {
            ...
            for (key, value) in strings {
                ...
                if let unit = en["stringUnit"] as? [String: Any] {
                    ...
                    let variations = try #require(en["variations"] as? [String: Any], "\(name)/\(key) en has neither stringUnit nor variations")
                    let plural = try #require(variations["plural"] as? [String: Any], "\(name)/\(key) en variations has no plural")
                    #expect(!plural.isEmpty, "\(name)/\(key) en plural has no categories")
                    for (category, variation) in plural {
                        let variant = try #require(variation as? [String: Any], "\(name)/\(key) plural \(category) malformed")
                        let unit = try #require(variant["stringUnit"] as? [String: Any], "\(name)/\(key) plural \(category) has no stringUnit")
                        let value = try #require(unit["value"] as? String, "\(name)/\(key) plural \(category) has no value")
