...
struct ContentViewModelTests {
    ...
    func taskStartsRecheckerAfterStoreStart() async {
        ...
        let viewModel = ContentViewModel(
            store: ReminderStore(eventStore: InMemoryEventStore(), loadsReminders: false),
            backgroundImage: BackgroundImageStore(),
            speechTranscriber: ReminderDictation()) { _ in recording }
        let task = Task { await viewModel.task(showUndatedReminders: true) }
        while recording.startCount == 0 {
            await Task.yield()
        }
        #expect(recording.startCount == 1)
        #expect(viewModel.store.showsUndatedReminders == true)
        task.cancel()
        await task.value
    }
    ...
    func taskCancellationStopsRecheckerOnce() async {
        ...
        let viewModel = ContentViewModel(
            ...
            backgroundImage: BackgroundImageStore(),
            speechTranscriber: ReminderDictation()) { _ in recording }
        let task = Task { await viewModel.task(showUndatedReminders: false) }
        while recording.startCount == 0 {
            await Task.yield()
        }
        task.cancel()
    ...
    func nilRecheckerFactoryDoesNotCrash() async {
        let viewModel = ContentViewModel(
            store: ReminderStore(eventStore: InMemoryEventStore(), loadsReminders: false),
            backgroundImage: BackgroundImageStore(),
            speechTranscriber: ReminderDictation()) { _ in nil }
        let task = Task { await viewModel.task(showUndatedReminders: true) }
        await Task.yield()
        task.cancel()
...
/// `task(_:)` can be asserted headlessly.
@MainActor
